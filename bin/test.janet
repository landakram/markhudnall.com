...
(defn test-load-content []
  ...
  (def loaded (content/load-content-from-vault @{}))
...
(defn test-duplicate-url-validation []
  ...
  (def old-vault (os/getenv "OBSIDIAN_VAULT"))
  (os/setenv "OBSIDIAN_VAULT" vault)
  (expect-error "duplicate URLs" (fn [] (content/load-content-from-vault @{})) "duplicate URL")
  (if old-vault
    (os/setenv "OBSIDIAN_VAULT" old-vault)
    (os/setenv "OBSIDIAN_VAULT" ""))
  (fs/remove-tree vault))

(defn test-public-snapshot-export []
  (def vault (temp-dir))
  (def snapshot (temp-dir))
  (fs/ensure-dir (path/join vault "Website" "Pages"))
  (fs/ensure-dir (path/join vault "Attachments" "Website"))
  (write (path/join vault "Website" "Public.base")
         "filters:\n  and:\n    - publish == true\n    - not:\n      - file.inFolder(\"Templates\")\n")
  (write (path/join vault "Website" "Pages" "Bio.md")
         "---\npublish: true\ntype: page\ntitle: Bio\nwebsite-role: bio\n---\n\nBio ![[logo.png]].\n")
  ...
  (write (path/join vault "Elsewhere.md")
         "---\npublish: true\ntype: page\ntitle: Elsewhere\nwebsite-role: elsewhere\n---\n\nElsewhere.\n")
  (write (path/join vault "Public Post.md")
         "---\npublish: true\ntype: post\ntitle: Public Post\ndate: 2024-01-05\nslug: public-post\n---\n\nBody.\n")
  (write (path/join vault "Private.md")
         "---\npublish: false\ntype: post\ntitle: Private\ndate: 2024-01-06\nslug: private\n---\n\nNope.\n")
  (fs/write-file (path/join vault "Attachments" "Website" "logo.png") "fake image")
  (def old-vault (os/getenv "OBSIDIAN_VAULT"))
  (def old-root (os/getenv "OBSIDIAN_CONTENT_ROOT"))
  (os/setenv "OBSIDIAN_VAULT" vault)
  (os/setenv "OBSIDIAN_CONTENT_ROOT" snapshot)
  (content/export-public-snapshot @{})
  (when (not (fs/exists? (path/join snapshot "Website" "Pages" "Bio.md")))
    (error "snapshot missing public page"))
  (when (fs/exists? (path/join snapshot "Private.md"))
    (error "snapshot included private note"))
  (when (not (fs/exists? (path/join snapshot "Attachments" "Website" "logo.png")))
    (error "snapshot missing referenced asset"))
  (def loaded (content/load-content-from-snapshot @{}))
  (assert= 1 (length (get loaded :posts)) "snapshot post count")
  (assert= 3 (length (get loaded :pages)) "snapshot page count")
  (if old-vault
    (os/setenv "OBSIDIAN_VAULT" old-vault)
    (os/setenv "OBSIDIAN_VAULT" ""))
  (if old-root
    (os/setenv "OBSIDIAN_CONTENT_ROOT" old-root)
    (os/setenv "OBSIDIAN_CONTENT_ROOT" ""))
  (fs/remove-tree vault)
  (fs/remove-tree snapshot))

(test-base-query)
(test-load-content)
(test-duplicate-url-validation)
(test-public-snapshot-export)
(print "Tests passed")
